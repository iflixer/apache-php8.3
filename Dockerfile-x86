...
RUN apt-get update \
    ...
    && apt-get -y --no-install-recommends install \
        apache2 \
        php-fpm php8.3-mysql php8.3-xml php8.3-gd php8.3-mbstring php8.3-bcmath php8.3-zip php-memcache php-redis \
        php8.3-curl \
        php-xdebug \
...
RUN cd /tmp \
    ...
    && mv ioncube/ioncube_loader_lin_8.3.so /usr/lib/php/20230831/ \
    && rm -Rf ioncube.tar.gz ioncube \
    && echo "zend_extension=/usr/lib/php/20230831/ioncube_loader_lin_8.3.so" > /etc/php/8.3/fpm/conf.d/00_docker-php-ext-ioncube_loader_lin_8.3.ini
...
# PHP Configuration
COPY config/php/fpm/php.ini /etc/php/8.3/fpm/php.ini
COPY config/php/fpm/pool.d/www.conf /etc/php/8.3/fpm/pool.d/www.conf
COPY config/php/fpm/xdebug.ini /etc/php/8.3/mods-available/xdebug.ini
